...
let auxConsumer = async()=>{
    ...
    await consumer.run({
        eachMessage: async ({topic, partition, message}) => {
            ...
            let temperature = {temp:message.value.toString(),timestamp:timestamp};
            ...
            mongoClient.connect(url,(err,db)=>{
                ...
                dbo.collection("temperatures").insertOne(temperature,(err,response)=>{
                    if (err) throw err;
...
app.get("/lastTemperature",(req,res)=>{
    ...
    mongoClient.connect(url,(err,db)=>{
        if(err) throw err;
        let dbo = db.db('stackPortal');
        let cursor = dbo.collection('temperatures').find().limit(1).sort({ $natural : -1 });
